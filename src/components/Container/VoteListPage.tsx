...
const VoteList = ({ data }: { data: Vote }) => {
  ...
  return (
    <>
      <Spacing size={30} />
      <Link href={data.active ? `/vote/${data.id}` : ''} className="block w-11/12 px-4 py-6 bg-white text-xl rounded-xl font-semibold text-gray-600 shadow-md" prefetch >
        <section className="" >
          <p>{data.title}</p>
          <p>투표에 참여해주세요.</p>
          <Spacing size={10} />
...
const ToastMessage = () => {
  ...
  return (
    <div id="toast-success" className="flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-main rounded-lg shadow" role="alert">
